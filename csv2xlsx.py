                ...
                for item in row:
                    ...
                    if (row[0] != 'ID'):
                        if ((colindex == 4)):
                            ...
                            if(item.find('\n')):
                                sheet.cell(row = rown, column = colindex).value = item.replace('\n','')
                            else:
...
if __name__ == '__main__':
    ...
    names = []
    if ((len(sys.argv) > 1) and (sys.argv[1] != '--help') and (len(names) != 0)):
        print('Running in arguments mode.')
